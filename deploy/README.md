...
On the server you can keep **two files** in one directory (e.g. `~/deploy/`): a copy of [`compose.ghcr.yml`](./compose.ghcr.yml) as `compose.yml`, and your env file (e.g. `secrets/solid.env`) with **`CLIENT_ENV_FILE`** pointing at that path.
...
```sh
docker compose -p solid -f compose.yml --env-file secrets/solid.env pull
docker compose -p solid -f compose.yml --env-file secrets/solid.env up -d
docker compose -p solid -f compose.yml --env-file secrets/solid.env logs -f erpnext
```
...
2. On the VPS: Docker + Compose; **`echo YOUR_GHCR_PAT | docker login ghcr.io -u AchuCelestter --password-stdin`** (GitHub user + PAT with **`read:packages`**).
...
docker compose -p solid -f compose.yml --env-file secrets/solid.env exec --user frappe erpnext bash -c "cd /home/frappe/bench && bench --site solid.celestter.com build"
```
